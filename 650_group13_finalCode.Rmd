---
title: "650_group13_finalCode"
author: "Shengyu Hung, Jennifer Guo, Mackenzie Mueller, Wenxin Ni, Haosheng Pu"
date: "2024-12-08"
...
# make sure "None" is a level for 'Depressed_cat'
...
variables <- c("Age", "Gender", "Race1", "BMI", "SleepTrouble", 
          "Depressed", "Smoke100", "PhysActiveDays", "Diabetes", 
          "Alcohol12PlusYr", "RegularMarij", "HardDrugs", "Poverty")
...
table_desc_stats <- participants %>%
  summarise(
    ...
    DepressedNone = paste0(sum(Depressed == "None"), " (", round(100 * sum(Depressed == "None") / n(), 1), "%)"),
    DepressedSeveral = paste0(sum(Depressed == "Several"), " (", round(100 * sum(Depressed == "Several") / n(), 1), "%)"),
    DepressedMost = paste0(sum(Depressed == "Most"), " (", round(100 * sum(Depressed == "Most") / n(), 1), "%)"),
    Smoke100_Yes = paste0(sum(Smoke100 == "Yes"), " (", round(100 * sum(Smoke100 == "Yes") / n(), 1), "%)"),
    PhysActiveDays = paste0(mean(PhysActiveDays), " (", sd(PhysActiveDays), ")"),
    ...
    Alcohol12PlusYr_Yes = paste0(sum(Alcohol12PlusYr == "Yes"), " (", round(100 * sum(Alcohol12PlusYr == "Yes") / n(), 1), "%)"),
    ...
    Poverty = paste0(mean(Poverty), " (", sd(Poverty), ")")
  )

kable(table_desc_stats, format = "markdown", align = "l")

# table_long <- table_desc_stats %>%
#   pivot_longer(cols = everything(), names_to = "Variable", values_to = "Description")
# 
# 
# kable(table_long, format = "markdown", align = "l")
