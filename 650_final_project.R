...
vars <- c("TotChol", "Age", "Gender", "Race1", "BMI", "SleepHrsNight_cat", 
          "Depressed_cat", "Smoke100", "PhysActive", "Diabetes")
...
NHANES_clean$Diabetes <- relevel(NHANES_clean$Diabetes, ref = "No")
...
predicted_chol <- lm(
  TotChol ~ Age_centered + BMI_centered + Gender + Race1 + 
    SleepHrsNight_cat + Depressed_cat + Smoke100 + PhysActive + Diabetes ,
  data = NHANES_clean
)
...
interaction_chol <- lm(
  TotChol ~ Age_centered + BMI_centered + Gender + Race1 + 
    SleepHrsNight_cat + Depressed_cat + Smoke100 + PhysActive + Diabetes + 
    SleepHrsNight_cat*Smoke100 + SleepHrsNight_cat*BMI_centered + 
    SleepHrsNight_cat*PhysActive + SleepHrsNight_cat*Diabetes,
  data = NHANES_clean
)

summary(interaction_chol)

vif_interaction <- vif(interaction_chol, type = 'predictor')
